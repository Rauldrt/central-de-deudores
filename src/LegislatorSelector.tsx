      ...
      <div className="p-4 border-b">
        ...
        <div className="mt-4 pt-3 border-t">
          ...
          <form className="flex gap-2" onSubmit={async (e) => {
            e.preventDefault();
            if (!cuitSearchTerm || cuitSearchTerm.length < 11) {
              setBcraError("Ingrese 11 dígitos");
              return;
            }
            try {
              setBcraError(null);
              setIsSearchingBcra(true);
              const customLegislator = await fetchBcraDeudas(cuitSearchTerm);
              // Asignar el slug como el CUIT para que fluya con el Dashboard
              onSelect({ ...customLegislator, slug: customLegislator.cuit } as any);
              setCuitSearchTerm('');
            } catch(e: any) {
              setBcraError(e.message);
            } finally {
              setIsSearchingBcra(false);
            }
          }}>
            <input
              id="search-cuit"
              className="w-full p-2 border rounded text-sm flex-1"
              placeholder="Ej: 20326896684"
            ...
            <button 
              type="submit" 
              disabled={isSearchingBcra}
              className="bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white px-3 py-1.5 rounded text-sm font-semibold transition"
            >
              {isSearchingBcra ? '...' : 'Buscar'}
            </button>
          </form>
