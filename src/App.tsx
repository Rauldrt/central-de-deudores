  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (personSlug) {
        ...
        if (!found && personSlug.length === 11 && /^\d+$/.test(personSlug)) {
          // If not in static dataset, try fetching from BCRA
          fetchBcraDeudas(personSlug).then((bcraLegislator) => {
            setPerson({ ...bcraLegislator, slug: bcraLegislator.cuit } as LegislatorWithSlug);
